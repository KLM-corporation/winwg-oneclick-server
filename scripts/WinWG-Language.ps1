<#
.SYNOPSIS
  Language helpers for WinWG OneClick Server.
#>

function Get-WinWGDefaultLanguage {
    try {
        $culture = [System.Globalization.CultureInfo]::CurrentUICulture.TwoLetterISOLanguageName.ToLowerInvariant()
        if ($culture -eq 'fr') { return 'fr' }
    } catch {}
    return 'en'
}

function Get-WinWGLanguagePath([string]$BaseDir) {
    return (Join-Path $BaseDir 'settings\language.txt')
}

function Get-WinWGLanguage([string]$BaseDir) {
    $path = Get-WinWGLanguagePath -BaseDir $BaseDir
    if (Test-Path $path) {
        $value = (Get-Content $path -Raw -ErrorAction SilentlyContinue).Trim().ToLowerInvariant()
        if ($value -in @('fr','en')) { return $value }
    }
    return Get-WinWGDefaultLanguage
}

function Set-WinWGLanguage([string]$BaseDir, [string]$Language) {
    $Language = $Language.Trim().ToLowerInvariant()
    if ($Language -notin @('fr','en')) { throw "Unsupported language: $Language" }
    $settingsDir = Join-Path $BaseDir 'settings'
    if (-not (Test-Path $settingsDir)) { New-Item -ItemType Directory -Path $settingsDir -Force | Out-Null }
    Set-Content -Path (Get-WinWGLanguagePath -BaseDir $BaseDir) -Value $Language -Encoding ASCII
    return $Language
}

function Select-WinWGLanguage([string]$BaseDir) {
    $default = Get-WinWGLanguage -BaseDir $BaseDir
    Write-Host ""
    Write-Host "Language / Langue" -ForegroundColor Cyan
    Write-Host "-----------------" -ForegroundColor DarkGray
    Write-Host "fr = Francais"
    Write-Host "en = English"
    Write-Host ""
    $choice = (Read-Host "Choose language / Choisir la langue [$default]").Trim().ToLowerInvariant()
    if ([string]::IsNullOrWhiteSpace($choice)) { $choice = $default }
    if ($choice -notin @('fr','en')) {
        Write-Host "Invalid language, using default: $default" -ForegroundColor Yellow
        $choice = $default
    }
    return (Set-WinWGLanguage -BaseDir $BaseDir -Language $choice)
}

function Get-WinWGText([string]$Language, [string]$Key) {
    $texts = @{
        fr = @{
            ConsoleTitle = 'WinWG OneClick Server - Console serveur unifiee'
            ConsoleSubtitle = 'Surveillance + controle du service VPN dans une seule console.'
            MenuNoAutoRefresh = 'Menu interactif: pas de rafraichissement automatique.'
            VerboseMode = 'Mode ultra verbeux'
            AdvancedMode = 'Mode avance'
            Installation = 'Installation'
            Service = 'Service'
            Tunnel = 'Tunnel'
            UdpPort = 'Port UDP'
            LocalIp = 'IP locale PC'
            ServerConfig = 'Config serveur'
            Firewall = 'Pare-feu'
            Nat = 'NAT Windows'
            UdpEndpoint = 'Endpoint UDP local'
            DeviceConfigs = 'Configs appareil'
            Actions = 'Actions'
            QuickHelp = 'Aide rapide'
            HelpPeerConnected = "Si l’appareil est connecte, il apparait dans 'Appareils / peers' avec un handshake recent."
            HelpSpeedRefresh = 'La vitesse RX/TX est calculee entre deux rafraichissements du statut. Utilise S pour mesurer.'
            StartVpn = 'Activer / demarrer le serveur VPN'
            StopVpn = 'Desactiver / arreter le serveur VPN'
            RestartVpn = 'Redemarrer le serveur VPN'
            AddDevice = 'Ajouter un nouvel appareil'
            RemoveDevice = 'Retirer / supprimer un appareil'
            GenerateQr = 'Generer un QR code pour un appareil'
            Refresh = 'Rafraichir le statut'
            ToggleVerbose = 'Activer/desactiver le mode ultra verbeux'
            AdvancedTools = 'Mode avance / outils experts'
            Language = 'Changer la langue'
            Quit = 'Quitter'
            Choice = 'Choix'
            LanguageChanged = 'Langue modifiee'
            Enabled = 'active'
            Disabled = 'desactive'
            Present = 'presente'
            Missing = 'manquant'
            NotInstalledDisabled = 'introuvable / desactive'
            AbsentUninstalled = 'absente / desinstallee'
            FirewallPresent = 'regle presente'
            FirewallMissing = 'regle manquante'
            UdpPresent = 'present'
            UdpNotVisible = 'non visible / normal selon driver'
            FileCount = 'fichier(s)'
            PhonesPeers = 'Appareils / peers'
            NoPeerDetected = 'Aucun appareil/peer detecte dans wg show.'
            Online = 'connecte'
            Offline = 'hors ligne'
            Never = 'jamais'
            VpnIp = 'IP VPN'
            Handshake = 'Handshake'
            Transfer = 'Transfert'
            Speed = 'Vitesse'
            PublicKey = 'Public key'
            SpeedNextRefresh = 'calcul au prochain refresh'
            DelayTooShort = 'delai trop court'
            RawWireGuardDetails = 'Details WireGuard bruts'
            UnknownDevice = 'appareil inconnu'
            FolderMissing = 'dossier introuvable'
            NoActiveTunnel = "WireGuard n'est pas actif, aucun handshake a afficher."
            StatusRefreshed = 'Statut rafraichi.'
            QrDisabled = "Option QR desactivee. Elle n'apparait pas dans le menu car la dependance QR n'a pas ete installee/activee."
            InvalidChoiceMain = 'Choix invalide. Utilise 1/2/3/4/5/6/7/8/9, A/D/N/R/G/K/E/H, I, S, V, M, L ou Q.'
            AdvancedWarningTitle = 'WinWG OneClick Server - Mode avance'
            Warning = 'ATTENTION'
            AdvancedWarningIntro1 = 'Le mode avance est destine aux personnes qui connaissent deja WireGuard.'
            AdvancedWarningIntro2 = 'Il peut donner acces a des actions et fichiers sensibles.'
            PossibleRisks = 'Risques possibles :'
            RiskBreakConfig = '- casser la configuration serveur ;'
            RiskExposeKey = '- exposer une cle privee si tu partages une capture ou un fichier ;'
            RiskCutAccess = '- couper l''acces VPN a tes appareils ;'
            RiskUnreachable = '- rendre le serveur inaccessible depuis l''exterieur.'
            NeverShareSecrets = 'Ne partage jamais les fichiers .conf, les QR codes, les cles privees ou les logs non relus.'
            AdvancedEnablePrompt = 'Pour activer, tape exactement JE COMPRENDS'
            AdvancedConfirmPhrase = 'JE COMPRENDS'
            AdvancedEnabled = 'Mode avance active. Sois prudent.'
            AdvancedNotEnabled = 'Mode avance non active.'
            AdvancedToolsTitle = 'WinWG OneClick Server - Outils avances'
            AdvancedToolsActive = 'Mode avance actif - attention aux cles et fichiers .conf'
            AdvRawWgShow = 'Afficher wg show brut'
            AdvOpenServerFolder = 'Ouvrir le dossier serveur'
            AdvOpenDevicesFolder = 'Ouvrir le dossier devices'
            AdvOpenQrFolder = 'Ouvrir le dossier QR codes'
            AdvExportDiagnostic = 'Exporter un diagnostic redige'
            AdvEditConfig = 'Modifier configuration avancee (port, DNS, AllowedIPs)'
            AdvOpenServerConf = 'Ouvrir winwg-server.conf dans Notepad (contient la cle privee)'
            AdvDisable = 'Desactiver le mode avance'
            Back = 'Retour'
            ServiceStarted = 'Service demarre'
            ServiceAlreadyActive = 'Service deja actif'
            ServiceInstalledStarted = 'Service installe et demarre'
            ServiceAlreadyDisabled = 'Service deja desactive / non installe'
            ServiceDisabled = 'Service desactive'
            ResidualServiceRemoved = 'Service residuel supprime'
            PressEnterReturn = 'Appuie sur Entree pour revenir au menu'
            Cancel = 'Annuler'
            GenerateQrTitle = 'Generer un QR code WireGuard'
            SingleDeviceDetected = 'Un seul appareil detecte'
            TypeNumberOrExactName = 'Tape le numero de l''appareil, ou son nom exact'
            RemoveDeviceTitle = 'Supprimer un appareil'
            SingleDeviceAuto = 'Un seul appareil est configure. Selection automatique'
            TypeNumberOrExactNameRemove = 'Tape le numero de l''appareil a supprimer, ou son nom exact'
            ConfirmRemove = 'Confirmer la suppression de'
            TypeOConfirm = 'Tape O pour confirmer'
            DeviceRemoved = 'Device supprime'
            AddDeviceTitle = 'Ajouter un appareil'
            DeviceNamePrompt = 'Nom de l''appareil, ex: iphone, android, laptop'
            EndpointPrompt = 'Endpoint public ou DNS'
            DnsPrompt = 'DNS pour cet appareil'
            KeepDefault = 'laisse vide pour garder cette valeur'
            AssignedVpnIp = 'IP VPN attribuee automatiquement'
            ReloadingService = 'Rechargement du service WireGuard...'
            WireGuardServiceReloaded = 'Service WireGuard recharge.'
            DeviceAdded = 'Appareil ajoute'
            VpnIpShort = 'IP VPN'
            DeviceConf = 'Config appareil'
            QrGenerated = 'QR code genere'
            ScanQr = "Scanne ce QR code avec l'application WireGuard sur Android/iOS."
            QrPrivateWarning = "Attention : ce QR contient la cle privee de l'appareil. Ne le partage pas publiquement."
            DeviceAddedFinal = 'Appareil ajoute'
            ConfGeneratedIn = 'Fichier .conf genere dans'
            QrGeneratedFor = 'QR code genere pour'
            AskGenerateQrForDevice = 'Generer un QR code pour cet appareil ?'
            QrSkippedForDevice = 'QR code non genere pour cet appareil.'
            QrSettings = 'Parametres QR code'
            QrFeatureStatus = 'Statut fonctionnalite QR'
            QrDependencyStatus = 'Statut dependance QRCoder'
            Installed = 'installee'
            NotInstalled = 'non installee'
            EnableQrFeature = 'Activer la fonctionnalite QR et installer QRCoder'
            DisableQrFeature = 'Desactiver la fonctionnalite QR'
            ReinstallQrDependency = 'Reinstaller/verifier QRCoder'
            OpenQrFolder = 'Ouvrir le dossier QR codes'
            QrFeatureEnabled = 'Fonctionnalite QR activee'
            QrFeatureDisabled = 'Fonctionnalite QR desactivee'
            QrDependencyInstalled = 'Dependance QRCoder installee'
            TemporaryDevice = 'Appareil temporaire'
            Temporary = 'Temporaire'
            ExpiresAt = 'Expire le'
            Remaining = 'Temps restant'
            Expired = 'expire'
            Permanent = 'permanent'
            AskTemporaryDevice = 'Creer cet appareil comme temporaire ?'
            TemporaryDurationMenu = 'Duree de validite'
            Duration1h = '1 - 1 heure'
            Duration6h = '2 - 6 heures'
            Duration24h = '3 - 24 heures'
            Duration7d = '4 - 7 jours'
            DurationCustomHours = '5 - Duree personnalisee en heures'
            TemporaryMetadataCreated = 'Metadonnees temporaires creees'
            DeviceMetadataCreated = 'Metadonnees appareil creees'
            RemoveExpiredDevices = 'Supprimer les appareils temporaires expires'
            HealthCheck = 'Health check / Diagnostic rapide'
            HealthTitle = 'WinWG - Health check'
            HealthOk = 'OK'
            HealthWarn = 'WARN'
            HealthError = 'ERREUR'
            HealthInfo = 'INFO'
            HealthSummary = 'Resume'
            HealthPassed = 'verification(s) OK'
            HealthWarnings = 'avertissement(s)'
            HealthErrors = 'erreur(s)'
            HealthAdvice = 'Conseil'
            HealthServiceRunning = 'Service WireGuard en cours d''execution'
            HealthServiceNotRunning = 'Service WireGuard absent ou arrete'
            HealthServerConfigPresent = 'Configuration serveur presente'
            HealthServerConfigMissing = 'Configuration serveur manquante'
            HealthFirewallPresent = 'Regle pare-feu presente'
            HealthFirewallMissing = 'Regle pare-feu manquante'
            HealthNatPresent = 'NAT Windows present'
            HealthNatMissing = 'NAT Windows manquant'
            HealthForwardingEnabled = 'Routage/forwarding IPv4 active'
            HealthForwardingDisabled = 'Routage/forwarding IPv4 a verifier'
            HealthDevicesFound = 'Configuration(s) appareil trouvee(s)'
            HealthNoDevices = 'Aucune configuration appareil trouvee'
            HealthPeersFound = 'Peer(s) WireGuard configure(s)'
            HealthNoPeers = 'Aucun peer WireGuard configure'
            HealthRecentHandshake = 'Handshake recent detecte'
            HealthNoRecentHandshake = 'Aucun handshake recent detecte'
            HealthUdpEndpointPresent = 'Endpoint UDP local visible'
            HealthUdpEndpointMissing = 'Endpoint UDP local non visible'
            HealthManualForward = 'Si l''appareil ne se connecte pas hors LAN, verifie la redirection manuelle : UDP {0} -> {1}:{0}'
            HealthPortForwardSucceeded = 'Redirection automatique detectee comme reussie'
            HealthPortForwardFailed = 'Redirection automatique non reussie / manuelle requise'
            HealthPortForwardUnknown = 'Statut redirection automatique inconnu'
            HealthPortForwardFile = 'Dernier statut redirection'
            HealthUpnpCurrentMappingPresent = 'Mapping UPnP actuel detecte'
            HealthUpnpCurrentMappingMissing = 'Mapping UPnP actuel non detectable'
            EndpointModeMenu = 'Mode endpoint IPv4/IPv6'
            EndpointModeStatus = 'Mode endpoint'
            EndpointModeTitle = 'Mode endpoint IPv4 / IPv6'
            CurrentEndpointMode = 'Mode actuel'
            SwitchToIPv6Endpoint = 'Basculer en endpoint IPv6 direct (peut demander une regle pare-feu IPv6 sur la box)'
            SwitchToIPv4Endpoint = 'Revenir en mode IPv4/DNS manuel pour les configs appareil'
            NoPublicIPv6Detected = 'Aucune IPv6 publique globale detectee sur ce PC. Mode IPv6 non active.'
            IPv6EndpointModeEnabled = 'Mode endpoint IPv6 direct active'
            IPv4EndpointModeEnabled = 'Mode endpoint IPv4/DNS active'
            DeviceConfigsUpdated = 'Configs appareil mises a jour'
            PortForwardStatusSaved = 'Statut redirection enregistre'
            ReimportDeviceConfigsWarning = 'Important : les appareils deja ajoutes doivent reimporter le .conf ou rescanner un QR regenere. Si ca marche en Wi-Fi mais pas en 4G/5G, cree une regle pare-feu IPv6 sur la box : UDP du port WireGuard vers l''adresse IPv6 du PC.'
            NoDeviceConfigFolder = 'Dossier des configs appareil introuvable'
            ErrorEndpointMode = 'ERREUR mode endpoint IPv4/IPv6'
            NoExpiredDevices = 'Aucun appareil temporaire expire trouve'
            ExpiredDevicesRemoved = 'Appareil(s) temporaire(s) expire(s) supprime(s)'
            ErrorActivation = 'ERREUR activation'
            ErrorDeactivation = 'ERREUR desactivation'
            ErrorRestart = 'ERREUR redemarrage'
            ErrorAddDevice = 'ERREUR ajout appareil'
            ErrorRemoveDevice = 'ERREUR suppression appareil'
            ErrorQr = 'ERREUR QR code'
            ErrorAdvanced = 'ERREUR mode avance'
        }
        en = @{
            ConsoleTitle = 'WinWG OneClick Server - Unified server console'
            ConsoleSubtitle = 'Monitoring + VPN service control in one console.'
            MenuNoAutoRefresh = 'Interactive menu: no automatic refresh.'
            VerboseMode = 'Ultra verbose mode'
            AdvancedMode = 'Advanced mode'
            Installation = 'Installation'
            Service = 'Service'
            Tunnel = 'Tunnel'
            UdpPort = 'UDP port'
            LocalIp = 'PC local IP'
            ServerConfig = 'Server config'
            Firewall = 'Firewall'
            Nat = 'Windows NAT'
            UdpEndpoint = 'Local UDP endpoint'
            DeviceConfigs = 'Config appareiligs'
            Actions = 'Actions'
            QuickHelp = 'Quick help'
            HelpPeerConnected = "If the device is connected, it appears in 'Devices / peers' with a recent handshake."
            HelpSpeedRefresh = 'RX/TX speed is calculated between two status refreshes. Press S to measure.'
            StartVpn = 'Enable / start VPN server'
            StopVpn = 'Disable / stop VPN server'
            RestartVpn = 'Restart VPN server'
            AddDevice = 'Add a new device'
            RemoveDevice = 'Remove a device'
            GenerateQr = 'Generate a QR code for a device'
            Refresh = 'Refresh status'
            ToggleVerbose = 'Enable/disable ultra verbose mode'
            AdvancedTools = 'Advanced mode / expert tools'
            Language = 'Change language'
            Quit = 'Quit'
            Choice = 'Choice'
            LanguageChanged = 'Language changed'
            Enabled = 'enabled'
            Disabled = 'disabled'
            Present = 'present'
            Missing = 'missing'
            NotInstalledDisabled = 'not installed / disabled'
            AbsentUninstalled = 'absent / uninstalled'
            FirewallPresent = 'rule present'
            FirewallMissing = 'rule missing'
            UdpPresent = 'present'
            UdpNotVisible = 'not visible / normal depending on driver'
            FileCount = 'file(s)'
            PhonesPeers = 'Devices / peers'
            NoPeerDetected = 'No device/peer detected in wg show.'
            Online = 'online'
            Offline = 'offline'
            Never = 'never'
            VpnIp = 'VPN IP'
            Handshake = 'Handshake'
            Transfer = 'Transfer'
            Speed = 'Speed'
            PublicKey = 'Public key'
            SpeedNextRefresh = 'calculated on next refresh'
            DelayTooShort = 'delay too short'
            RawWireGuardDetails = 'Raw WireGuard details'
            UnknownDevice = 'unknown device'
            FolderMissing = 'folder not found'
            NoActiveTunnel = 'WireGuard is not active, no handshake to display.'
            StatusRefreshed = 'Status refreshed.'
            QrDisabled = 'QR option disabled. It is hidden from the menu because the QR dependency was not installed/enabled.'
            InvalidChoiceMain = 'Invalid choice. Use 1/2/3/4/5/6/7/8/9, A/D/N/R/G/K/E/H, I, S, V, M, L or Q.'
            AdvancedWarningTitle = 'WinWG OneClick Server - Advanced mode'
            Warning = 'WARNING'
            AdvancedWarningIntro1 = 'Advanced mode is intended for users who already understand WireGuard.'
            AdvancedWarningIntro2 = 'It can expose sensitive actions and files.'
            PossibleRisks = 'Possible risks:'
            RiskBreakConfig = '- break the server configuration;'
            RiskExposeKey = '- expose a private key if you share a screenshot or file;'
            RiskCutAccess = '- cut VPN access for your devices;'
            RiskUnreachable = '- make the server unreachable from outside.'
            NeverShareSecrets = 'Never share .conf files, QR codes, private keys, or logs you have not reviewed.'
            AdvancedEnablePrompt = 'To enable, type exactly I UNDERSTAND'
            AdvancedConfirmPhrase = 'I UNDERSTAND'
            AdvancedEnabled = 'Advanced mode enabled. Be careful.'
            AdvancedNotEnabled = 'Advanced mode not enabled.'
            AdvancedToolsTitle = 'WinWG OneClick Server - Advanced tools'
            AdvancedToolsActive = 'Advanced mode enabled - be careful with keys and .conf files'
            AdvRawWgShow = 'Show raw wg show output'
            AdvOpenServerFolder = 'Open server folder'
            AdvOpenDevicesFolder = 'Open devices folder'
            AdvOpenQrFolder = 'Open QR codes folder'
            AdvExportDiagnostic = 'Export redacted diagnostic'
            AdvEditConfig = 'Edit advanced configuration (port, DNS, AllowedIPs)'
            AdvOpenServerConf = 'Open winwg-server.conf in Notepad (contains private key)'
            AdvDisable = 'Disable advanced mode'
            Back = 'Back'
            ServiceStarted = 'Service started'
            ServiceAlreadyActive = 'Service already active'
            ServiceInstalledStarted = 'Service installed and started'
            ServiceAlreadyDisabled = 'Service already disabled / not installed'
            ServiceDisabled = 'Service disabled'
            ResidualServiceRemoved = 'Residual service removed'
            PressEnterReturn = 'Press Enter to return to the menu'
            Cancel = 'Cancel'
            GenerateQrTitle = 'Generate a WireGuard QR code'
            SingleDeviceDetected = 'Single device detected'
            TypeNumberOrExactName = 'Type the device number, or its exact name'
            RemoveDeviceTitle = 'Remove a device'
            SingleDeviceAuto = 'Only one device is configured. Automatic selection'
            TypeNumberOrExactNameRemove = 'Type the number of the device to remove, or its exact name'
            ConfirmRemove = 'Confirm removal of'
            TypeOConfirm = 'Type O to confirm'
            DeviceRemoved = 'Device removed'
            AddDeviceTitle = 'Add a device'
            DeviceNamePrompt = 'Device name, e.g. iphone, android, laptop'
            EndpointPrompt = 'Public endpoint or DNS'
            DnsPrompt = 'DNS for this device'
            KeepDefault = 'leave empty to keep this value'
            AssignedVpnIp = 'Automatically assigned VPN IP'
            ReloadingService = 'Reloading WireGuard service...'
            WireGuardServiceReloaded = 'WireGuard service reloaded.'
            DeviceAdded = 'Device added'
            VpnIpShort = 'VPN IP'
            DeviceConf = 'Config appareilig'
            QrGenerated = 'QR code generated'
            ScanQr = 'Scan this QR code with the WireGuard app on Android/iOS.'
            QrPrivateWarning = 'Warning: this QR contains the device private key. Do not share it publicly.'
            DeviceAddedFinal = 'Device added'
            ConfGeneratedIn = '.conf file generated in'
            QrGeneratedFor = 'QR code generated for'
            AskGenerateQrForDevice = 'Generate a QR code for this device?'
            QrSkippedForDevice = 'QR code not generated for this device.'
            QrSettings = 'QR code settings'
            QrFeatureStatus = 'QR feature status'
            QrDependencyStatus = 'QRCoder dependency status'
            Installed = 'installed'
            NotInstalled = 'not installed'
            EnableQrFeature = 'Enable QR feature and install QRCoder'
            DisableQrFeature = 'Disable QR feature'
            ReinstallQrDependency = 'Reinstall/check QRCoder'
            OpenQrFolder = 'Open QR codes folder'
            QrFeatureEnabled = 'QR feature enabled'
            QrFeatureDisabled = 'QR feature disabled'
            QrDependencyInstalled = 'QRCoder dependency installed'
            TemporaryDevice = 'Temporary device'
            Temporary = 'Temporary'
            ExpiresAt = 'Expires at'
            Remaining = 'Remaining'
            Expired = 'expired'
            Permanent = 'permanent'
            AskTemporaryDevice = 'Create this device as temporary?'
            TemporaryDurationMenu = 'Validity duration'
            Duration1h = '1 - 1 hour'
            Duration6h = '2 - 6 hours'
            Duration24h = '3 - 24 hours'
            Duration7d = '4 - 7 days'
            DurationCustomHours = '5 - Custom duration in hours'
            TemporaryMetadataCreated = 'Temporary metadata created'
            DeviceMetadataCreated = 'Device metadata created'
            RemoveExpiredDevices = 'Remove expired temporary devices'
            HealthCheck = 'Health check'
            HealthTitle = 'WinWG - Health check'
            HealthOk = 'OK'
            HealthWarn = 'WARN'
            HealthError = 'ERROR'
            HealthInfo = 'INFO'
            HealthSummary = 'Summary'
            HealthPassed = 'check(s) OK'
            HealthWarnings = 'warning(s)'
            HealthErrors = 'error(s)'
            HealthAdvice = 'Advice'
            HealthServiceRunning = 'WireGuard service is running'
            HealthServiceNotRunning = 'WireGuard service missing or stopped'
            HealthServerConfigPresent = 'Server configuration present'
            HealthServerConfigMissing = 'Server configuration missing'
            HealthFirewallPresent = 'Firewall rule present'
            HealthFirewallMissing = 'Firewall rule missing'
            HealthNatPresent = 'Windows NAT present'
            HealthNatMissing = 'Windows NAT missing'
            HealthForwardingEnabled = 'IPv4 routing/forwarding enabled'
            HealthForwardingDisabled = 'IPv4 routing/forwarding should be checked'
            HealthDevicesFound = 'Device configuration(s) found'
            HealthNoDevices = 'No device configuration found'
            HealthPeersFound = 'WireGuard peer(s) configured'
            HealthNoPeers = 'No WireGuard peer configured'
            HealthRecentHandshake = 'Recent handshake detected'
            HealthNoRecentHandshake = 'No recent handshake detected'
            HealthUdpEndpointPresent = 'Local UDP endpoint visible'
            HealthUdpEndpointMissing = 'Local UDP endpoint not visible'
            HealthManualForward = 'If the device cannot connect outside LAN, check manual forwarding: UDP {0} -> {1}:{0}'
            HealthPortForwardSucceeded = 'Automatic port forwarding recorded as successful'
            HealthPortForwardFailed = 'Automatic port forwarding not successful / manual rule required'
            HealthPortForwardUnknown = 'Automatic port forwarding status unknown'
            HealthPortForwardFile = 'Last port-forwarding status'
            HealthUpnpCurrentMappingPresent = 'Current UPnP mapping detected'
            HealthUpnpCurrentMappingMissing = 'Current UPnP mapping not detectable'
            EndpointModeMenu = 'IPv4/IPv6 endpoint mode'
            EndpointModeStatus = 'Endpoint mode'
            EndpointModeTitle = 'IPv4 / IPv6 endpoint mode'
            CurrentEndpointMode = 'Current mode'
            SwitchToIPv6Endpoint = 'Switch to direct IPv6 endpoint (may require an IPv6 firewall rule on the router)'
            SwitchToIPv4Endpoint = 'Return to manual IPv4/DNS mode for device configs'
            NoPublicIPv6Detected = 'No global public IPv6 address detected on this PC. IPv6 mode not enabled.'
            IPv6EndpointModeEnabled = 'Direct IPv6 endpoint mode enabled'
            IPv4EndpointModeEnabled = 'IPv4/DNS endpoint mode enabled'
            DeviceConfigsUpdated = 'Device configs updated'
            PortForwardStatusSaved = 'Port-forwarding status saved'
            ReimportDeviceConfigsWarning = 'Important: already-added devices must reimport the .conf or rescan a regenerated QR code. If it works on Wi-Fi but not on 4G/5G, create an IPv6 firewall rule on the router: the WireGuard UDP port to the PC IPv6 address.'
            NoDeviceConfigFolder = 'Device config folder not found'
            ErrorEndpointMode = 'ERROR IPv4/IPv6 endpoint mode'
            NoExpiredDevices = 'No expired temporary device found'
            ExpiredDevicesRemoved = 'Expired temporary device(s) removed'
            ErrorActivation = 'ERROR enabling'
            ErrorDeactivation = 'ERROR disabling'
            ErrorRestart = 'ERROR restarting'
            ErrorAddDevice = 'ERROR adding device'
            ErrorRemoveDevice = 'ERROR removing device'
            ErrorQr = 'ERROR QR code'
            ErrorAdvanced = 'ERROR advanced mode'
        }
    }
    if (-not $texts.ContainsKey($Language)) { $Language = 'en' }
    if ($texts[$Language].ContainsKey($Key)) { return $texts[$Language][$Key] }
    return $Key
}
